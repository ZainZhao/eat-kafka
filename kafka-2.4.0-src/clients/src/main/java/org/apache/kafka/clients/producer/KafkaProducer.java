...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    // 如果没有明确指定client的id，则使用字段生成一个ID，线程安全
    ...
    // 消息的最大长度，这长度包含了消息头、序列化后的key和序列化后的value的长度
    ...
    // 用于收集并缓存消息，等待 Sender 线程发送
    private final RecordAccumulator accumulator;
    // 发送消息的 Sender 任务，在 ioThread 线程中执行
    ...
    // ProducerInterceptor 集合 ，可以优先于用户的 Callback，对ACK相应进行预处理
    ...
    KafkaProducer(Map<String, Object> configs,
        ...
        try {
            ...
            // 根据 用户自定义的 clientId 和 transactionalId 生成
